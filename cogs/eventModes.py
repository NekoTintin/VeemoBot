...
class EventModes(commands.Cog):
    
    def __init__(self, bot) -> None:
        self.bot = bot
        
        bot.loop.create_task(self.get_match_data())
        #bot.loop.create_task(self.get_salmon_data())
    ...
    async def get_match_data(self) -> None:
        ...
        while not self.bot.is_closed():
            ...
            driver = scrap.Chrome(options=opts)
            driver.get(var.challenge_url)
            text_shadow_list = driver.find_elements("css selector", 'div.text-shadow')
            ...
            if dt.now(tzone) >= dt.strptime(f"{text_shadow_list[5].text[19:]} 2023", "%d/%m %H:%M %Y").astimezone(tzone):
                with open("/home/Tintin/discord_bot/Veemo/data/last_challenge_match_id.txt", "r") as file:
                    await self.bot.get_guild(1027361429529034843).fetch_schedule_event(file.read()).delete()
                
                    
            if not event_already_created:
                font_splatoon2_list = driver.find_elements("css selector", 'div.font-splatoon2')
                ...
                with open("/home/Tintin/discord_bot/Veemo/data/last_challenge_match_id.txt", "w") as file:
                    file.write(event.id)
    ...
    """
    async def get_salmon_data(self) -> None:
        await self.bot.wait_until_ready()
        event_already_created = False
...
async def setup(bot):
    await bot.add_cog(EventModes(bot))